  ...
  return function(src, options) {
    ...
    function parseRoot() {
      ...
      while (curr) {
        ...
        skipBlanks();
        if (lookahead("struct")) {
          var stmt = { type: "StructDefinition", member: [], pos };
          ...
          var stmt = { type: "EnumDefinition", member: [], pos };
          stmt.name = readIdentifier();
          consume("{");
    ...
    function parseStatement() {
      ...
      if (lookahead("return")) {
        ...
        var stmt = { type: "DoWhileStatement", pos };
        stmt.body = parseBody();
        consume("while");
        consume("(");
        stmt.condition = parseExpression(")");
        ...
        var stmt = { type: "ForStatement", pos };
        consume("(");
...
var c_plugin = await plugin(_c_plugin);
var plugin_default = c_plugin;
async function extract_symbols(path) {
  const contents = await Bun.file(path).text();
  const prepared = contents.replaceAll(/#include\s*<.*>/g, "");
  const ast = cparse(prepared, { file: path });
  const result = {};
  function toFFIType(t) {
  ...
  for (const node of ast) {
    if (node.type === "FunctionDeclaration" || node.type === "FunctionDefinition") {
      result[node.name] = {
        args: (node.arguments || []).map((arg) => toFFIType(arg.defType)),
...
function type_helper_for(symbols) {
  const typeMap = {
    0: "FFIType.char",
    1: "FFIType.int8_t",
    2: "FFIType.uint8_t",
  ...
  for (const [name, ffi] of Object.entries(symbols)) {
    const args = ffi.args.map((arg) => typeMap[arg] || "unknown").map((a, idx) => `_${idx}: ` + a).join(", ");
